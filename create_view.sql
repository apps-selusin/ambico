...
CREATE VIEW `v_keg_hasil` AS select `t_keg_master`.`kegm_id` AS `kegm_id`,`t_keg_master`.`keg_id` AS `keg_id`,`t_keg_master`.`tgl` AS `tgl`,`t_keg_master`.`shift` AS `shift`,`t_keg_master`.`hasil` AS `hasil`,`t_keg_detail`.`kegd_id` AS `kegd_id`,`t_kegiatan`.`keg_nama` AS `keg_nama`,`t_kegiatan`.`tarif_acuan` AS `tarif_acuan`,`t_kegiatan`.`tarif1` AS `tarif1`,`t_kegiatan`.`tarif2` AS `tarif2`,`t_kegiatan`.`keg_ket` AS `keg_ket`,`v_keg_pembagi`.`pembagi` AS `pembagi`,`pegawai`.`pegawai_nama` AS `pegawai_nama`,round(((case when (`t_kegiatan`.`tarif_acuan` = 0) then (`t_keg_master`.`hasil` * `t_kegiatan`.`tarif1`) else ((case when (`t_keg_master`.`hasil` <= `t_kegiatan`.`tarif_acuan`) then `t_kegiatan`.`tarif1` else `t_kegiatan`.`tarif2` end) * `t_keg_master`.`hasil`) end) / `v_keg_pembagi`.`pembagi`),0) AS `upah_peg`,concat(cast(`t_keg_master`.`tgl` as char charset utf8),' ',cast(`t_keg_master`.`hasil` as char charset utf8)) AS `header_` from ((((`t_keg_master` join `t_keg_detail` on((`t_keg_master`.`kegm_id` = `t_keg_detail`.`kegm_id`))) join `t_kegiatan` on((`t_keg_master`.`keg_id` = `t_kegiatan`.`keg_id`))) join `v_keg_pembagi` on((`t_keg_master`.`kegm_id` = `v_keg_pembagi`.`kegm_id`))) join `pegawai` on((`t_keg_detail`.`pegawai_id` = `pegawai`.`pegawai_id`)));
