...
$t_lembur_delete = NULL; // Initialize page object first

class ct_lembur_delete extends ct_lembur {
	...
	// Page object name
	var $PageObjName = 't_lembur_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("t_lemburlist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in t_lembur class, t_lemburinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("t_lemburlist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($t_lembur_delete)) $t_lembur_delete = new ct_lembur_delete();

// Page init
$t_lembur_delete->Page_Init();

// Page main
$t_lembur_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_lembur_delete->Page_Render();
?>
...
var CurrentForm = ft_lemburdelete = new ew_Form("ft_lemburdelete", "delete");

// Form_CustomValidate event
ft_lemburdelete.Form_CustomValidate = 
...
ft_lemburdelete.ValidateRequired = true;
<?php } else { ?>
ft_lemburdelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_lemburdelete.Lists["x_pegawai_id"] = {"LinkField":"x_pegawai_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_pegawai_nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"pegawai"};
...
<?php $t_lembur_delete->ShowPageHeader(); ?>
<?php
$t_lembur_delete->ShowMessage();
?>
<form name="ft_lemburdelete" id="ft_lemburdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_lembur_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_lembur_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="t_lembur">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($t_lembur_delete->RecKeys as $key) { ?>
...
<?php if ($t_lembur->pegawai_id->Visible) { // pegawai_id ?>
		<th><span id="elh_t_lembur_pegawai_id" class="t_lembur_pegawai_id"><?php echo $t_lembur->pegawai_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($t_lembur->tgl_mulai->Visible) { // tgl_mulai ?>
		<th><span id="elh_t_lembur_tgl_mulai" class="t_lembur_tgl_mulai"><?php echo $t_lembur->tgl_mulai->FldCaption() ?></span></th>
<?php } ?>
<?php if ($t_lembur->tgl_selesai->Visible) { // tgl_selesai ?>
		<th><span id="elh_t_lembur_tgl_selesai" class="t_lembur_tgl_selesai"><?php echo $t_lembur->tgl_selesai->FldCaption() ?></span></th>
<?php } ?>
<?php if ($t_lembur->jam_mulai->Visible) { // jam_mulai ?>
		<th><span id="elh_t_lembur_jam_mulai" class="t_lembur_jam_mulai"><?php echo $t_lembur->jam_mulai->FldCaption() ?></span></th>
<?php } ?>
<?php if ($t_lembur->jam_selesai->Visible) { // jam_selesai ?>
		<th><span id="elh_t_lembur_jam_selesai" class="t_lembur_jam_selesai"><?php echo $t_lembur->jam_selesai->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$t_lembur_delete->RecCnt = 0;
$i = 0;
while (!$t_lembur_delete->Recordset->EOF) {
	$t_lembur_delete->RecCnt++;
	$t_lembur_delete->RowCnt++;

	// Set row properties
	$t_lembur->ResetAttrs();
	$t_lembur->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$t_lembur_delete->LoadRowValues($t_lembur_delete->Recordset);

	// Render row
	$t_lembur_delete->RenderRow();
?>
	<tr<?php echo $t_lembur->RowAttributes() ?>>
<?php if ($t_lembur->pegawai_id->Visible) { // pegawai_id ?>
		<td<?php echo $t_lembur->pegawai_id->CellAttributes() ?>>
<span id="el<?php echo $t_lembur_delete->RowCnt ?>_t_lembur_pegawai_id" class="t_lembur_pegawai_id">
...
<?php } ?>
<?php if ($t_lembur->tgl_mulai->Visible) { // tgl_mulai ?>
		<td<?php echo $t_lembur->tgl_mulai->CellAttributes() ?>>
<span id="el<?php echo $t_lembur_delete->RowCnt ?>_t_lembur_tgl_mulai" class="t_lembur_tgl_mulai">
...
<?php } ?>
<?php if ($t_lembur->tgl_selesai->Visible) { // tgl_selesai ?>
		<td<?php echo $t_lembur->tgl_selesai->CellAttributes() ?>>
<span id="el<?php echo $t_lembur_delete->RowCnt ?>_t_lembur_tgl_selesai" class="t_lembur_tgl_selesai">
...
<?php } ?>
<?php if ($t_lembur->jam_mulai->Visible) { // jam_mulai ?>
		<td<?php echo $t_lembur->jam_mulai->CellAttributes() ?>>
<span id="el<?php echo $t_lembur_delete->RowCnt ?>_t_lembur_jam_mulai" class="t_lembur_jam_mulai">
...
<?php } ?>
<?php if ($t_lembur->jam_selesai->Visible) { // jam_selesai ?>
		<td<?php echo $t_lembur->jam_selesai->CellAttributes() ?>>
<span id="el<?php echo $t_lembur_delete->RowCnt ?>_t_lembur_jam_selesai" class="t_lembur_jam_selesai">
...
<?php
	$t_lembur_delete->Recordset->MoveNext();
}
$t_lembur_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_lembur_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ft_lemburdelete.Init();
</script>
<?php
$t_lembur_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_lembur_delete->Page_Terminate();
?>
