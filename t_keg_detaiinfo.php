...
//
// Table class for t_keg_detai
//
class ct_keg_detai extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 't_keg_detai';
		$this->TableName = 't_keg_detai';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t_keg_detai`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// kegd_id
		$this->kegd_id = new cField('t_keg_detai', 't_keg_detai', 'x_kegd_id', 'kegd_id', '`kegd_id`', '`kegd_id`', 3, -1, FALSE, '`kegd_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// pegawai_id
		$this->pegawai_id = new cField('t_keg_detai', 't_keg_detai', 'x_pegawai_id', 'pegawai_id', '`pegawai_id`', '`pegawai_id`', 3, -1, FALSE, '`EV__pegawai_id`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'SELECT');
		...
		// kegm_id
		$this->kegm_id = new cField('t_keg_detai', 't_keg_detai', 'x_kegm_id', 'kegm_id', '`kegm_id`', '`kegm_id`', 3, -1, FALSE, '`kegm_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`t_keg_detai`";
	}
	...
	function getSqlSelectList() { // Select for List page
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT `pegawai_nama` FROM `pegawai` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`pegawai_id` = `t_keg_detai`.`pegawai_id` LIMIT 1) AS `EV__pegawai_id` FROM `t_keg_detai`" .
			") `EW_TMP_TABLE`";
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t_keg_detaiview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t_keg_detaiview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "t_keg_detaiadd.php?" . $this->UrlParm($parm);
		else
			$url = "t_keg_detaiadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("t_keg_detaiedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("t_keg_detaiadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("t_keg_detaidelete.php", $this->UrlParm());
	}
