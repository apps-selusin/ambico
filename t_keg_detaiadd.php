...
$t_keg_detai_add = NULL; // Initialize page object first

class ct_keg_detai_add extends ct_keg_detai {
	...
	// Page object name
	var $PageObjName = 't_keg_detai_add';
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("t_keg_detailist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "t_keg_detaiview.php")
...
// Create page object
if (!isset($t_keg_detai_add)) $t_keg_detai_add = new ct_keg_detai_add();

// Page init
$t_keg_detai_add->Page_Init();

// Page main
$t_keg_detai_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_keg_detai_add->Page_Render();
?>
...
var CurrentForm = ft_keg_detaiadd = new ew_Form("ft_keg_detaiadd", "add");

// Validate form
ft_keg_detaiadd.Validate = function() {
...
ft_keg_detaiadd.Form_CustomValidate = 
...
ft_keg_detaiadd.ValidateRequired = true;
<?php } else { ?>
ft_keg_detaiadd.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $t_keg_detai_add->ShowPageHeader(); ?>
<?php
$t_keg_detai_add->ShowMessage();
?>
<form name="ft_keg_detaiadd" id="ft_keg_detaiadd" class="<?php echo $t_keg_detai_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_keg_detai_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_keg_detai_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_keg_detai_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
ft_keg_detaiadd.Init();
</script>
<?php
$t_keg_detai_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_keg_detai_add->Page_Terminate();
?>
