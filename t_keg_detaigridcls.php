...
$t_keg_detai_grid = NULL; // Initialize page object first

class ct_keg_detai_grid extends ct_keg_detai {
	...
	// Page object name
	var $PageObjName = 't_keg_detai_grid';

	// Grid form hidden field names
	var $FormName = 'ft_keg_detaigrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["t_keg_detai"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->kegd_id->CurrentValue;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->kegd_id->CurrentValue . "\">";
		}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->kegd_id->CurrentValue = strval($arKeys[0]); // kegd_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "t_keg_master") {
			$this->kegm_id->Visible = FALSE;
			if ($GLOBALS["t_keg_master"]->EventCancelled) $this->EventCancelled = TRUE;
		}
