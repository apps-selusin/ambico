...
$t_keg_detai_delete = NULL; // Initialize page object first

class ct_keg_detai_delete extends ct_keg_detai {
	...
	// Page object name
	var $PageObjName = 't_keg_detai_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("t_keg_detailist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in t_keg_detai class, t_keg_detaiinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("t_keg_detailist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($t_keg_detai_delete)) $t_keg_detai_delete = new ct_keg_detai_delete();

// Page init
$t_keg_detai_delete->Page_Init();

// Page main
$t_keg_detai_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_keg_detai_delete->Page_Render();
?>
...
var CurrentForm = ft_keg_detaidelete = new ew_Form("ft_keg_detaidelete", "delete");

// Form_CustomValidate event
ft_keg_detaidelete.Form_CustomValidate = 
...
ft_keg_detaidelete.ValidateRequired = true;
<?php } else { ?>
ft_keg_detaidelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft_keg_detaidelete.Lists["x_pegawai_id"] = {"LinkField":"x_pegawai_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_pegawai_nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"pegawai"};
...
<?php $t_keg_detai_delete->ShowPageHeader(); ?>
<?php
$t_keg_detai_delete->ShowMessage();
?>
<form name="ft_keg_detaidelete" id="ft_keg_detaidelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_keg_detai_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_keg_detai_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="t_keg_detai">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($t_keg_detai_delete->RecKeys as $key) { ?>
...
<?php } ?>
	...
	<tbody>
<?php
$t_keg_detai_delete->RecCnt = 0;
$i = 0;
while (!$t_keg_detai_delete->Recordset->EOF) {
	$t_keg_detai_delete->RecCnt++;
	$t_keg_detai_delete->RowCnt++;

	// Set row properties
	$t_keg_detai->ResetAttrs();
	$t_keg_detai->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$t_keg_detai_delete->LoadRowValues($t_keg_detai_delete->Recordset);

	// Render row
	$t_keg_detai_delete->RenderRow();
?>
	<tr<?php echo $t_keg_detai->RowAttributes() ?>>
<?php if ($t_keg_detai->pegawai_id->Visible) { // pegawai_id ?>
		<td<?php echo $t_keg_detai->pegawai_id->CellAttributes() ?>>
...
<?php
	$t_keg_detai_delete->Recordset->MoveNext();
}
$t_keg_detai_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_keg_detai_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ft_keg_detaidelete.Init();
</script>
<?php
$t_keg_detai_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_keg_detai_delete->Page_Terminate();
?>
