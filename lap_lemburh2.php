...
function f_hitungjamlembur_old($p_conn, $p_pegawai_id) {
	$query = "select * from t_lembur2 where pegawai_id = ".$p_pegawai_id." order by tgl"; //$query = "select * from t_lembur where pegawai_id = ".$p_pegawai_id." order by tgl_mulai";
	$rs = $p_conn->Execute($query);
	$mlama_lembur = 0;
	while (!$rs->EOF) {
		$mtgl_mulai = $rs->fields["tgl_mulai"];
