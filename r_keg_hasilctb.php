...
<?php include_once "r_keg_hasilctbinfo.php" ?>
<?php
...
$r_keg_hasil_crosstab = NULL; // Initialize page object first

class crr_keg_hasil_crosstab extends crr_keg_hasil {
	...
	// Page object name
	var $PageObjName = 'r_keg_hasil_crosstab';
	...
	function __construct() {
		...
		// Table object (r_keg_hasil)
		if (!isset($GLOBALS["r_keg_hasil"])) {
			$GLOBALS["r_keg_hasil"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["r_keg_hasil"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'r_keg_hasil', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fr_keg_hasilcrosstab";
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 'r_keg_hasil');
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_r_keg_hasil\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_r_keg_hasil',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fr_keg_hasilcrosstab\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fr_keg_hasilcrosstab\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fr_keg_hasilcrosstab\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function GetExtendedFilter() {
		...
		// Reset search command
			...
			$this->SetSessionDropDownValue($this->keg_nama->DropDownValue, $this->keg_nama->SearchOperator, 'keg_nama'); // Field keg_nama
			...
			// Field keg_nama
			if ($this->GetDropDownValue($this->keg_nama)) {
				$bSetupFilter = TRUE;
			} elseif ($this->keg_nama->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_r_keg_hasil_keg_nama'])) {
				$bSetupFilter = TRUE;
			}
			...
			if ($this->GetDropDownValue($this->pegawai_nama)) {
				$bSetupFilter = TRUE;
			} elseif ($this->pegawai_nama->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_r_keg_hasil_pegawai_nama'])) {
				$bSetupFilter = TRUE;
			}
		...
		if ($bRestoreSession) {
			...
			$this->GetSessionDropDownValue($this->keg_nama); // Field keg_nama
		...
		$this->BuildDropDownFilter($this->keg_nama, $sFilter, $this->keg_nama->SearchOperator, FALSE, TRUE); // Field keg_nama
		...
		// Field keg_nama
		ewr_LoadDropDownList($this->keg_nama->DropDownList, $this->keg_nama->DropDownValue);
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_r_keg_hasil_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_r_keg_hasil_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_r_keg_hasil_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_r_keg_hasil_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_r_keg_hasil_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_r_keg_hasil_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_r_keg_hasil_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_r_keg_hasil_' . $parm] = $sv;
		$_SESSION['so_r_keg_hasil_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_r_keg_hasil_' . $parm] = $sv1;
		$_SESSION['so_r_keg_hasil_' . $parm] = $so1;
		$_SESSION['sc_r_keg_hasil_' . $parm] = $sc;
		$_SESSION['sv2_r_keg_hasil_' . $parm] = $sv2;
		$_SESSION['so2_r_keg_hasil_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_r_keg_hasil_$parm"] = "";
		$_SESSION["rf_r_keg_hasil_$parm"] = "";
		$_SESSION["rt_r_keg_hasil_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->FieldByParm($parm);
		$fld->SelectionList = @$_SESSION["sel_r_keg_hasil_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_r_keg_hasil_$parm"];
		$fld->RangeTo = @$_SESSION["rt_r_keg_hasil_$parm"];
	}
	...
	function LoadDefaultFilters() {
		...
		*/
		...
		// Field keg_nama
		$this->keg_nama->DefaultDropDownValue = EWR_INIT_VALUE;
		if (!$this->SearchCommand) $this->keg_nama->DropDownValue = $this->keg_nama->DefaultDropDownValue;
	...
	function ShowFilterList($showDate = FALSE) {
		...
		$sExtWrk = "";
		$sWrk = "";
		$this->BuildDropDownFilter($this->keg_nama, $sExtWrk, $this->keg_nama->SearchOperator);
		$sFilter = "";
		if ($sExtWrk <> "")
			$sFilter .= "<span class=\"ewFilterValue\">$sExtWrk</span>";
		elseif ($sWrk <> "")
			$sFilter .= "<span class=\"ewFilterValue\">$sWrk</span>";
		if ($sFilter <> "")
			$sFilterList .= "<div><span class=\"ewFilterCaption\">" . $this->keg_nama->FldCaption() . "</span>" . $sFilter . "</div>";
	...
	function GetFilterList() {
		...
		// Field keg_nama
		$sWrk = "";
		$sWrk = ($this->keg_nama->DropDownValue <> EWR_INIT_VALUE) ? $this->keg_nama->DropDownValue : "";
		if (is_array($sWrk))
			$sWrk = implode("||", $sWrk);
		if ($sWrk <> "")
			$sWrk = "\"sv_keg_nama\":\"" . ewr_JsEncode2($sWrk) . "\"";
		if ($sWrk <> "") {
			if ($sFilterList <> "") $sFilterList .= ",";
			$sFilterList .= $sWrk;
		}
	...
	function SetupFilterList($filter) {
		...
		// Field keg_nama
		$bRestoreFilter = FALSE;
		if (array_key_exists("sv_keg_nama", $filter)) {
			$sWrk = $filter["sv_keg_nama"];
			if (strpos($sWrk, "||") !== FALSE)
				$sWrk = explode("||", $sWrk);
			$this->SetSessionDropDownValue($sWrk, @$filter["so_keg_nama"], "keg_nama");
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionDropDownValue(EWR_INIT_VALUE, "", "keg_nama");
		}
...
// Create page object
if (!isset($r_keg_hasil_crosstab)) $r_keg_hasil_crosstab = new crr_keg_hasil_crosstab();
if (isset($Page)) $OldPage = $Page;
$Page = &$r_keg_hasil_crosstab;
...
// Create page object
var r_keg_hasil_crosstab = new ewr_Page("r_keg_hasil_crosstab");

// Page properties
r_keg_hasil_crosstab.PageID = "crosstab"; // Page ID
var EWR_PAGE_ID = r_keg_hasil_crosstab.PageID;

// Extend page with Chart_Rendering function
r_keg_hasil_crosstab.Chart_Rendering = 
...
r_keg_hasil_crosstab.Chart_Rendered = 
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
// Form object
var CurrentForm = fr_keg_hasilcrosstab = new ewr_Form("fr_keg_hasilcrosstab");

// Validate method
fr_keg_hasilcrosstab.Validate = function() {
...
fr_keg_hasilcrosstab.Form_CustomValidate = 
...
fr_keg_hasilcrosstab.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fr_keg_hasilcrosstab.ValidateRequired = false; // No JavaScript validation
<?php } ?>

// Use Ajax
fr_keg_hasilcrosstab.Lists["sv_keg_nama"] = {"LinkField":"sv_keg_nama","Ajax":true,"DisplayFields":["sv_keg_nama","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
fr_keg_hasilcrosstab.Lists["sv_pegawai_nama"] = {"LinkField":"sv_pegawai_nama","Ajax":true,"DisplayFields":["sv_pegawai_nama","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
</script>
<?php } ?>
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<script type="text/javascript">
...
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<!-- Search form (begin) -->
<form name="fr_keg_hasilcrosstab" id="fr_keg_hasilcrosstab" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fr_keg_hasilcrosstab_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<input type="text" data-table="r_keg_hasil" data-field="x_tgl" id="sv_tgl" name="sv_tgl" placeholder="<?php echo $Page->tgl->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->tgl->SearchValue) ?>" data-calendar="true" data-formatid="0"<?php echo $Page->tgl->EditAttributes() ?>>
</span>
	<span class="ewSearchCond btw1_tgl"><?php echo $ReportLanguage->Phrase("AND") ?></span>
	<span class="ewSearchField btw1_tgl">
<?php ewr_PrependClass($Page->tgl->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" data-table="r_keg_hasil" data-field="x_tgl" id="sv2_tgl" name="sv2_tgl" placeholder="<?php echo $Page->tgl->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->tgl->SearchValue2) ?>" data-calendar="true" data-formatid="0"<?php echo $Page->tgl->EditAttributes() ?>>
</span>
</div>
</div>
<div id="r_2" class="ewRow">
<div id="c_keg_nama" class="ewCell form-group">
	<label for="sv_keg_nama" class="ewSearchCaption ewLabel"><?php echo $Page->keg_nama->FldCaption() ?></label>
	<span class="ewSearchField">
<?php ewr_PrependClass($Page->keg_nama->EditAttrs["class"], "form-control"); ?>
<select data-table="r_keg_hasil" data-field="x_keg_nama" data-value-separator="<?php echo ewr_HtmlEncode(is_array($Page->keg_nama->DisplayValueSeparator) ? json_encode($Page->keg_nama->DisplayValueSeparator) : $Page->keg_nama->DisplayValueSeparator) ?>" id="sv_keg_nama" name="sv_keg_nama"<?php echo $Page->keg_nama->EditAttributes() ?>>
<option value=""><?php echo $ReportLanguage->Phrase("PleaseSelect") ?></option>
<?php
	$cntf = is_array($Page->keg_nama->AdvancedFilters) ? count($Page->keg_nama->AdvancedFilters) : 0;
	$cntd = is_array($Page->keg_nama->DropDownList) ? count($Page->keg_nama->DropDownList) : 0;
	$totcnt = $cntf + $cntd;
	$wrkcnt = 0;
	if ($cntf > 0) {
		foreach ($Page->keg_nama->AdvancedFilters as $filter) {
			if ($filter->Enabled) {
				$selwrk = ewr_MatchedFilterValue($Page->keg_nama->DropDownValue, $filter->ID) ? " selected" : "";
?>
...
<?php
	...
	for ($i = 0; $i < $cntd; $i++) {
		$selwrk = " selected";
?>
<option value="<?php echo $Page->keg_nama->DropDownList[$i] ?>"<?php echo $selwrk ?>><?php echo ewr_DropDownDisplayValue($Page->keg_nama->DropDownList[$i], "", 0) ?></option>
<?php
		$wrkcnt += 1;
	}
?>
</select>
<input type="hidden" name="s_sv_keg_nama" id="s_sv_keg_nama" value="<?php echo $Page->keg_nama->LookupFilterQuery() ?>"></span>
</div>
</div>
...
<div id="c_pegawai_nama" class="ewCell form-group">
	...
	<span class="ewSearchField">
<?php ewr_PrependClass($Page->pegawai_nama->EditAttrs["class"], "form-control"); ?>
<select data-table="r_keg_hasil" data-field="x_pegawai_nama" data-value-separator="<?php echo ewr_HtmlEncode(is_array($Page->pegawai_nama->DisplayValueSeparator) ? json_encode($Page->pegawai_nama->DisplayValueSeparator) : $Page->pegawai_nama->DisplayValueSeparator) ?>" id="sv_pegawai_nama" name="sv_pegawai_nama"<?php echo $Page->pegawai_nama->EditAttributes() ?>>
<option value=""><?php echo $ReportLanguage->Phrase("PleaseSelect") ?></option>
<?php
	$cntf = is_array($Page->pegawai_nama->AdvancedFilters) ? count($Page->pegawai_nama->AdvancedFilters) : 0;
	$cntd = is_array($Page->pegawai_nama->DropDownList) ? count($Page->pegawai_nama->DropDownList) : 0;
	$totcnt = $cntf + $cntd;
	$wrkcnt = 0;
	if ($cntf > 0) {
		foreach ($Page->pegawai_nama->AdvancedFilters as $filter) {
			if ($filter->Enabled) {
				$selwrk = ewr_MatchedFilterValue($Page->pegawai_nama->DropDownValue, $filter->ID) ? " selected" : "";
?>
<option value="<?php echo $filter->ID ?>"<?php echo $selwrk ?>><?php echo $filter->Name ?></option>
<?php
				$wrkcnt += 1;
	...
	for ($i = 0; $i < $cntd; $i++) {
		$selwrk = " selected";
?>
<option value="<?php echo $Page->pegawai_nama->DropDownList[$i] ?>"<?php echo $selwrk ?>><?php echo ewr_DropDownDisplayValue($Page->pegawai_nama->DropDownList[$i], "", 0) ?></option>
<?php
		$wrkcnt += 1;
	}
?>
...
<script type="text/javascript">
fr_keg_hasilcrosstab.Init();
fr_keg_hasilcrosstab.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<?php include "r_keg_hasilctbpager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="keg_nama">
		<div class="r_keg_hasil_keg_nama"><span class="ewTableHeaderCaption"><?php echo $Page->keg_nama->FldCaption() ?></span></div>
	</td>
<?php } else { ?>
	<td data-field="keg_nama">
<?php if ($Page->SortUrl($Page->keg_nama) == "") { ?>
		<div class="ewTableHeaderBtn r_keg_hasil_keg_nama">
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="pegawai_nama">
		<div class="r_keg_hasil_pegawai_nama"><span class="ewTableHeaderCaption"><?php echo $Page->pegawai_nama->FldCaption() ?></span></div>
	</td>
<?php } else { ?>
	<td data-field="pegawai_nama">
<?php if ($Page->SortUrl($Page->pegawai_nama) == "") { ?>
		<div class="ewTableHeaderBtn r_keg_hasil_pegawai_nama">
			<span class="ewTableHeaderCaption"><?php echo $Page->pegawai_nama->FldCaption() ?></span>			
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_keg_hasil_pegawai_nama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->pegawai_nama) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->pegawai_nama->FldCaption() ?></span>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_r_keg_hasil_pegawai_nama"<?php echo $Page->pegawai_nama->ViewAttributes() ?>><?php echo $Page->pegawai_nama->GroupViewValue ?></span></td>
<?php } ?>
<!-- Dynamic columns begin -->
<?php
