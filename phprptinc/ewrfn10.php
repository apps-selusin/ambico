...
function CurrentProjectID() {
	...
	return "{6A79AFFA-AA3A-4CBB-8572-5F6C56B1E5B1}";
}
...
class crAdvancedSecurity {
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($validateUser) {
						...
						$this->setSessionParentUserID($rs->fields('user_id')); // Load parent User ID
	...
	function LoadUserID() {
		...
		if (strval($this->CurrentUserID) == "") {
			...
			$sParentFilter = '`user_id` IN (' . $this->UserIDList() . ')';
			...
			if (EWR_USER_ID_IS_HIERARCHICAL) {
				...
				while ($sUserIDList <> $sCurUserIDList) {
					$sFilter = '`user_id` IN (' . $sCurUserIDList . ')';
					$sSql = EWR_LOGIN_SELECT_SQL . " WHERE " . $sFilter;
					if ($RsUser = $UserTableConn->Execute($sSql)) {
						while (!$RsUser->EOF) {
							$this->AddUserID($RsUser->fields('user_id'));
	...
	function ParentUserIDList($userid) {
		...
		if (trim(strval($userid)) == strval(CurrentUserID())) {
			if (strval(CurrentParentUserID()) <> "")
				$result = ewr_QuotedValue(CurrentParentUserID(), EWR_DATATYPE_NUMBER, EWR_USER_TABLE_DBID);
			return $result;
		}

		// One level only, must be CurrentUserID
		if (!EWR_USER_ID_IS_HIERARCHICAL) {
			return ewr_QuotedValue(CurrentUserID(), EWR_DATATYPE_NUMBER, EWR_USER_TABLE_DBID);
			...
			for ($i = 0; $i < $len; $i++) {
				if (strval($ar[$i]) <> trim(strval($userid))) {
					if ($result <> "")
						$result .= ", ";
					$result .= ewr_QuotedValue($ar[$i], EWR_DATATYPE_NUMBER, EWR_USER_TABLE_DBID);
				}
			}
			return $result;
		}
...
function Database_Connecting(&$info) {
	...
	if (ewr_CurrentUserIP () == "127.0.0.1"  || ewr_CurrentUserIP () == "::1"  || ewr_CurrentHost () == "localhost" ) { // testing on local PC
		...
		$info["user"] = "u945388674_ambi2"; // sesuaikan dengan username database di komputer server
		$info["pass"] = "M457r1P 81"; // sesuaikan deengan password database di komputer server
		$info["db"] = "u945388674_ambi2"; // sesuaikan dengan nama database di komputer server
