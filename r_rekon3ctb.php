...
<?php include_once "r_rekon3ctbinfo.php" ?>
<?php
...
$r_rekon3_crosstab = NULL; // Initialize page object first

class crr_rekon3_crosstab extends crr_rekon3 {
	...
	// Page object name
	var $PageObjName = 'r_rekon3_crosstab';
	...
	function __construct() {
		...
		// Table object (r_rekon3)
		if (!isset($GLOBALS["r_rekon3"])) {
			$GLOBALS["r_rekon3"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["r_rekon3"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'r_rekon3', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fr_rekon3crosstab";
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 'r_rekon3');
		...
		$this->pembagian2_nama->PlaceHolder = $this->pembagian2_nama->FldCaption();
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_r_rekon3\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_r_rekon3',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fr_rekon3crosstab\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fr_rekon3crosstab\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fr_rekon3crosstab\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function SetupPopup() {
		...
		// Get 'reset' command
			...
			if (strtolower($sCmd) == "reset") {
				$_SESSION["sel_r_rekon3_tgl"] = "";
				$_SESSION["rf_r_rekon3_tgl"] = "";
				$_SESSION["rt_r_rekon3_tgl"] = "";
				$this->ResetPager();
			}
		}

		// Load selection criteria to array
		if (is_array(@$_SESSION["sel_r_rekon3_tgl"])) {
			$this->tgl->SelectionList = @$_SESSION["sel_r_rekon3_tgl"];
			$this->tgl->RangeFrom = @$_SESSION["rf_r_rekon3_tgl"];
			$this->tgl->RangeTo = @$_SESSION["rt_r_rekon3_tgl"];
		} elseif (@$_SESSION["sel_r_rekon3_tgl"] == EWR_INIT_VALUE) { // Select all
			$this->tgl->SelectionList = "";
		}
	}
	...
	function GetExtendedFilter() {
		...
		if ($bPostBack) {
			...
			if ($this->ClearExtFilter == 'r_rekon3_tgl')
		...
		// Reset search command
			...
			$this->SetSessionFilterValues($this->pembagian2_nama->SearchValue, $this->pembagian2_nama->SearchOperator, $this->pembagian2_nama->SearchCondition, $this->pembagian2_nama->SearchValue2, $this->pembagian2_nama->SearchOperator2, 'pembagian2_nama'); // Field pembagian2_nama
			...
			if ($this->GetFilterValues($this->pembagian2_nama)) {
				$bSetupFilter = TRUE;
			}
		...
		if ($bRestoreSession) {
			...
			$this->GetSessionFilterValues($this->pembagian2_nama); // Field pembagian2_nama
		}
		...
		$this->BuildExtendedFilter($this->pembagian2_nama, $sFilter, FALSE, TRUE); // Field pembagian2_nama
		...
		if ($bSetupFilter) {
			...
			$_SESSION['sel_r_rekon3_tgl'] = ($this->tgl->SelectionList == "") ? EWR_INIT_VALUE : $this->tgl->SelectionList;
		}
		return $sFilter;
	}
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_r_rekon3_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_r_rekon3_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_r_rekon3_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_r_rekon3_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_r_rekon3_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_r_rekon3_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_r_rekon3_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_r_rekon3_' . $parm] = $sv;
		$_SESSION['so_r_rekon3_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_r_rekon3_' . $parm] = $sv1;
		$_SESSION['so_r_rekon3_' . $parm] = $so1;
		$_SESSION['sc_r_rekon3_' . $parm] = $sc;
		$_SESSION['sv2_r_rekon3_' . $parm] = $sv2;
		$_SESSION['so2_r_rekon3_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_r_rekon3_$parm"] = "";
		$_SESSION["rf_r_rekon3_$parm"] = "";
		$_SESSION["rt_r_rekon3_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->FieldByParm($parm);
		$fld->SelectionList = @$_SESSION["sel_r_rekon3_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_r_rekon3_$parm"];
		$fld->RangeTo = @$_SESSION["rt_r_rekon3_$parm"];
	}
	...
	function LoadDefaultFilters() {
		...
		*/
		...
		// Field pembagian2_nama
		$this->SetDefaultExtFilter($this->pembagian2_nama, "LIKE", NULL, 'AND', "=", NULL);
		if (!$this->SearchCommand) $this->ApplyDefaultExtFilter($this->pembagian2_nama);
		/**
	...
	function CheckFilter() {
		...
		// Check pembagian2_nama text filter
		if ($this->TextFilterApplied($this->pembagian2_nama))
			return TRUE;
	...
	function ShowFilterList($showDate = FALSE) {
		...
		$sExtWrk = "";
		$sWrk = "";
		$this->BuildExtendedFilter($this->pembagian2_nama, $sExtWrk);
		$sFilter = "";
		if ($sExtWrk <> "")
			$sFilter .= "<span class=\"ewFilterValue\">$sExtWrk</span>";
		elseif ($sWrk <> "")
			$sFilter .= "<span class=\"ewFilterValue\">$sWrk</span>";
		if ($sFilter <> "")
			$sFilterList .= "<div><span class=\"ewFilterCaption\">" . $this->pembagian2_nama->FldCaption() . "</span>" . $sFilter . "</div>";
		$divstyle = "";
		$divdataclass = "";
	...
	function GetFilterList() {
		...
		// Field pembagian2_nama
		$sWrk = "";
		if ($this->pembagian2_nama->SearchValue <> "" || $this->pembagian2_nama->SearchValue2 <> "") {
			$sWrk = "\"sv_pembagian2_nama\":\"" . ewr_JsEncode2($this->pembagian2_nama->SearchValue) . "\"," .
				"\"so_pembagian2_nama\":\"" . ewr_JsEncode2($this->pembagian2_nama->SearchOperator) . "\"," .
				"\"sc_pembagian2_nama\":\"" . ewr_JsEncode2($this->pembagian2_nama->SearchCondition) . "\"," .
				"\"sv2_pembagian2_nama\":\"" . ewr_JsEncode2($this->pembagian2_nama->SearchValue2) . "\"," .
				"\"so2_pembagian2_nama\":\"" . ewr_JsEncode2($this->pembagian2_nama->SearchOperator2) . "\"";
		}
		if ($sWrk <> "") {
			if ($sFilterList <> "") $sFilterList .= ",";
			$sFilterList .= $sWrk;
	...
	function SetupFilterList($filter) {
		...
		if (array_key_exists("sel_tgl", $filter)) {
			...
			$_SESSION["sel_r_rekon3_tgl"] = $sWrk;
		...
		// Field pembagian2_nama
		$bRestoreFilter = FALSE;
		if (array_key_exists("sv_pembagian2_nama", $filter) || array_key_exists("so_pembagian2_nama", $filter) ||
			array_key_exists("sc_pembagian2_nama", $filter) ||
			array_key_exists("sv2_pembagian2_nama", $filter) || array_key_exists("so2_pembagian2_nama", $filter)) {
			$this->SetSessionFilterValues(@$filter["sv_pembagian2_nama"], @$filter["so_pembagian2_nama"], @$filter["sc_pembagian2_nama"], @$filter["sv2_pembagian2_nama"], @$filter["so2_pembagian2_nama"], "pembagian2_nama");
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "pembagian2_nama");
		}
		return TRUE;
	}
...
// Create page object
if (!isset($r_rekon3_crosstab)) $r_rekon3_crosstab = new crr_rekon3_crosstab();
if (isset($Page)) $OldPage = $Page;
$Page = &$r_rekon3_crosstab;
...
// Create page object
var r_rekon3_crosstab = new ewr_Page("r_rekon3_crosstab");

// Page properties
r_rekon3_crosstab.PageID = "crosstab"; // Page ID
var EWR_PAGE_ID = r_rekon3_crosstab.PageID;

// Extend page with Chart_Rendering function
r_rekon3_crosstab.Chart_Rendering = 
...
r_rekon3_crosstab.Chart_Rendered = 
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
// Form object
var CurrentForm = fr_rekon3crosstab = new ewr_Form("fr_rekon3crosstab");

// Validate method
fr_rekon3crosstab.Validate = function() {
...
fr_rekon3crosstab.Form_CustomValidate = 
...
fr_rekon3crosstab.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fr_rekon3crosstab.ValidateRequired = false; // No JavaScript validation
<?php } ?>
...
ewr_CreatePopup("r_rekon3_tgl", <?php echo $jsdb ?>); // Popup filters
</script>
<?php } ?>
<?php if ($Page->Export == "") { ?>
<!-- container (begin) -->
...
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<!-- Search form (begin) -->
<form name="fr_rekon3crosstab" id="fr_rekon3crosstab" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fr_rekon3crosstab_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<input type="text" data-table="r_rekon3" data-field="x_tgl" id="sv_tgl" name="sv_tgl" placeholder="<?php echo $Page->tgl->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->tgl->SearchValue) ?>" data-calendar="true" data-formatid="0"<?php echo $Page->tgl->EditAttributes() ?>>
</span>
	<span class="ewSearchCond btw1_tgl"><?php echo $ReportLanguage->Phrase("AND") ?></span>
	<span class="ewSearchField btw1_tgl">
<?php ewr_PrependClass($Page->tgl->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" data-table="r_rekon3" data-field="x_tgl" id="sv2_tgl" name="sv2_tgl" placeholder="<?php echo $Page->tgl->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->tgl->SearchValue2) ?>" data-calendar="true" data-formatid="0"<?php echo $Page->tgl->EditAttributes() ?>>
</span>
</div>
</div>
...
<input type="text" data-table="r_rekon3" data-field="x_pegawai_nama" id="sv_pegawai_nama" name="sv_pegawai_nama" size="30" maxlength="255" placeholder="<?php echo $Page->pegawai_nama->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->pegawai_nama->SearchValue) ?>"<?php echo $Page->pegawai_nama->EditAttributes() ?>>
</span>
</div>
</div>
<div id="r_3" class="ewRow">
<div id="c_pembagian2_nama" class="ewCell form-group">
	<label for="sv_pembagian2_nama" class="ewSearchCaption ewLabel"><?php echo $Page->pembagian2_nama->FldCaption() ?></label>
	<span class="ewSearchOperator"><?php echo $ReportLanguage->Phrase("LIKE"); ?><input type="hidden" name="so_pembagian2_nama" id="so_pembagian2_nama" value="LIKE"></span>
	<span class="control-group ewSearchField">
<?php ewr_PrependClass($Page->pembagian2_nama->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" data-table="r_rekon3" data-field="x_pembagian2_nama" id="sv_pembagian2_nama" name="sv_pembagian2_nama" size="30" maxlength="100" placeholder="<?php echo $Page->pembagian2_nama->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->pembagian2_nama->SearchValue) ?>"<?php echo $Page->pembagian2_nama->EditAttributes() ?>>
</span>
</div>
</div>
...
<script type="text/javascript">
fr_rekon3crosstab.Init();
fr_rekon3crosstab.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<?php include "r_rekon3ctbpager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php } ?>
...
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
				<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'r_rekon3_tgl', true, '<?php echo $Page->tgl->RangeFrom ?>', '<?php echo $Page->tgl->RangeTo ?>');" name="x_tgl" id="x_tgl"><span class="icon-filter"></span></a>
<?php } ?>
			</div>
		</td>
	</tr>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="pembagian2_nama">
		<div class="r_rekon3_pembagian2_nama"><span class="ewTableHeaderCaption"><?php echo $Page->pembagian2_nama->FldCaption() ?></span></div>
	</td>
<?php } else { ?>
	<td data-field="pembagian2_nama">
<?php if ($Page->SortUrl($Page->pembagian2_nama) == "") { ?>
		<div class="ewTableHeaderBtn r_rekon3_pembagian2_nama">
			<span class="ewTableHeaderCaption"><?php echo $Page->pembagian2_nama->FldCaption() ?></span>			
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_rekon3_pembagian2_nama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->pembagian2_nama) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->pembagian2_nama->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="pegawai_nama">
		<div class="r_rekon3_pegawai_nama"><span class="ewTableHeaderCaption"><?php echo $Page->pegawai_nama->FldCaption() ?></span></div>
	</td>
<?php } else { ?>
	<td data-field="pegawai_nama">
<?php if ($Page->SortUrl($Page->pegawai_nama) == "") { ?>
		<div class="ewTableHeaderBtn r_rekon3_pegawai_nama">
			<span class="ewTableHeaderCaption"><?php echo $Page->pegawai_nama->FldCaption() ?></span>			
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_rekon3_pegawai_nama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->pegawai_nama) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->pegawai_nama->FldCaption() ?></span>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_r_rekon3_pembagian2_nama"<?php echo $Page->pembagian2_nama->ViewAttributes() ?>><?php echo $Page->pembagian2_nama->GroupViewValue ?></span></td>
<?php } ?>
<?php if ($Page->pegawai_nama->Visible) { ?>
		<!-- pegawai_nama -->
		<td data-field="pegawai_nama"<?php echo $Page->pegawai_nama->CellAttributes(); ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_r_rekon3_pegawai_nama"<?php echo $Page->pegawai_nama->ViewAttributes() ?>><?php echo $Page->pegawai_nama->GroupViewValue ?></span></td>
<?php } ?>
<!-- Dynamic columns begin -->
<?php
