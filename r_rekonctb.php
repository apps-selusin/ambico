...
//		$item->Visible = TRUE;
	...
	function GetExtendedFilter() {
		...
		if ($bPostBack) {
			...
			if ($this->ClearExtFilter == 'r_rekon_tgl')
		...
		// Reset search command
			...
			// Load default values
			$this->SetSessionDropDownValue($this->pembagian2_nama->DropDownValue, $this->pembagian2_nama->SearchOperator, 'pembagian2_nama'); // Field pembagian2_nama
			...
			if ($this->GetDropDownValue($this->pembagian2_nama)) {
				$bSetupFilter = TRUE;
			} elseif ($this->pembagian2_nama->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_r_rekon_pembagian2_nama'])) {
				$bSetupFilter = TRUE;
			}
		...
		// Restore session
		if ($bRestoreSession) {
			$this->GetSessionDropDownValue($this->pembagian2_nama); // Field pembagian2_nama
		...
		// Build SQL
		$this->BuildDropDownFilter($this->pembagian2_nama, $sFilter, $this->pembagian2_nama->SearchOperator, FALSE, TRUE); // Field pembagian2_nama
		...
		if ($bSetupFilter) {
			...
			$_SESSION['sel_r_rekon_tgl'] = ($this->tgl->SelectionList == "") ? EWR_INIT_VALUE : $this->tgl->SelectionList;
		}

		// Field pembagian2_nama
		ewr_LoadDropDownList($this->pembagian2_nama->DropDownList, $this->pembagian2_nama->DropDownValue);
	...
	function LoadDefaultFilters() {
		...
		*/
		...
		// Field pembagian2_nama
		$this->pembagian2_nama->DefaultDropDownValue = EWR_INIT_VALUE;
		if (!$this->SearchCommand) $this->pembagian2_nama->DropDownValue = $this->pembagian2_nama->DefaultDropDownValue;
	...
	function CheckFilter() {
		...
		// Check pembagian2_nama extended filter
		if ($this->NonTextFilterApplied($this->pembagian2_nama))
			return TRUE;
	...
	function ShowFilterList($showDate = FALSE) {
		...
		$sExtWrk = "";
		$sWrk = "";
		$this->BuildDropDownFilter($this->pembagian2_nama, $sExtWrk, $this->pembagian2_nama->SearchOperator);
		$sFilter = "";
		if ($sExtWrk <> "")
	...
	function GetFilterList() {
		...
		// Field pembagian2_nama
		$sWrk = "";
		$sWrk = ($this->pembagian2_nama->DropDownValue <> EWR_INIT_VALUE) ? $this->pembagian2_nama->DropDownValue : "";
		if (is_array($sWrk))
			$sWrk = implode("||", $sWrk);
		if ($sWrk <> "")
			$sWrk = "\"sv_pembagian2_nama\":\"" . ewr_JsEncode2($sWrk) . "\"";
		if ($sWrk <> "") {
	...
	function SetupFilterList($filter) {
		...
		if (array_key_exists("sv_pembagian2_nama", $filter)) {
			$sWrk = $filter["sv_pembagian2_nama"];
			if (strpos($sWrk, "||") !== FALSE)
				$sWrk = explode("||", $sWrk);
			$this->SetSessionDropDownValue($sWrk, @$filter["so_pembagian2_nama"], "pembagian2_nama");
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionDropDownValue(EWR_INIT_VALUE, "", "pembagian2_nama");
		}
...
// Use Ajax
fr_rekoncrosstab.Lists["sv_pembagian2_nama"] = {"LinkField":"sv_pembagian2_nama","Ajax":true,"DisplayFields":["sv_pembagian2_nama","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
...
ewr_CreatePopup("r_rekon_tgl", <?php echo $jsdb ?>); // Popup filters
</script>
<?php } ?>
<?php if ($Page->Export == "") { ?>
<!-- container (begin) -->
...
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next().click();" tabindex="-1" class="form-control ewLookupText" id="lu_sv_pembagian2_nama"><?php echo (strval(ewr_FilterDropDownValue($Page->pembagian2_nama)) == "" ? $ReportLanguage->Phrase("PleaseSelect") : ewr_FilterDropDownValue($Page->pembagian2_nama)); ?></span>
</span>
<button type="button" title="<?php echo ewr_HtmlEncode(str_replace("%s", ewr_RemoveHtml($Page->pembagian2_nama->FldCaption()), $ReportLanguage->Phrase("LookupLink", TRUE))) ?>" onclick="ewr_ModalLookupShow({lnk:this,el:'sv_pembagian2_nama',m:0,n:10});" class="ewLookupBtn btn btn-default btn-sm"><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" data-table="r_rekon" data-field="x_pembagian2_nama" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $Page->pembagian2_nama->DisplayValueSeparatorAttribute() ?>" name="sv_pembagian2_nama" id="sv_pembagian2_nama" value="<?php echo ewr_FilterDropDownValue($Page->pembagian2_nama, ",") ?>"<?php echo $Page->pembagian2_nama->EditAttributes() ?>>
<input type="hidden" name="s_sv_pembagian2_nama" id="s_sv_pembagian2_nama" value="<?php echo $Page->pembagian2_nama->LookupFilterQuery() ?>"></span>
</div>
</div>
...
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
				<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'r_rekon_tgl', true, '<?php echo $Page->tgl->RangeFrom ?>', '<?php echo $Page->tgl->RangeTo ?>');" name="x_tgl" id="x_tgl"><span class="icon-filter"></span></a>
<?php } ?>
			</div>
		</td>
	</tr>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="pembagian2_nama">
		<div class="r_rekon_pembagian2_nama"><span class="ewTableHeaderCaption"><?php echo $Page->pembagian2_nama->FldCaption() ?></span></div>
	</td>
<?php } else { ?>
	<td data-field="pembagian2_nama">
<?php if ($Page->SortUrl($Page->pembagian2_nama) == "") { ?>
		<div class="ewTableHeaderBtn r_rekon_pembagian2_nama">
			<span class="ewTableHeaderCaption"><?php echo $Page->pembagian2_nama->FldCaption() ?></span>			
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_rekon_pembagian2_nama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->pembagian2_nama) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->pembagian2_nama->FldCaption() ?></span>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_r_rekon_pembagian2_nama"<?php echo $Page->pembagian2_nama->ViewAttributes() ?>><?php echo $Page->pembagian2_nama->GroupViewValue ?></span></td>
<?php } ?>
<?php if ($Page->pegawai_nama->Visible) { ?>
		<!-- pegawai_nama -->
