...
$t_keg_master_delete = NULL; // Initialize page object first

class ct_keg_master_delete extends ct_keg_master {
	...
	// Page object name
	var $PageObjName = 't_keg_master_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("t_keg_masterlist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in t_keg_master class, t_keg_masterinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("t_keg_masterlist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($t_keg_master_delete)) $t_keg_master_delete = new ct_keg_master_delete();

// Page init
$t_keg_master_delete->Page_Init();

// Page main
$t_keg_master_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_keg_master_delete->Page_Render();
?>
...
var CurrentForm = ft_keg_masterdelete = new ew_Form("ft_keg_masterdelete", "delete");

// Form_CustomValidate event
ft_keg_masterdelete.Form_CustomValidate = 
...
ft_keg_masterdelete.ValidateRequired = true;
<?php } else { ?>
ft_keg_masterdelete.ValidateRequired = false; 
<?php } ?>
...
<?php $t_keg_master_delete->ShowPageHeader(); ?>
<?php
$t_keg_master_delete->ShowMessage();
?>
<form name="ft_keg_masterdelete" id="ft_keg_masterdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_keg_master_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_keg_master_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="t_keg_master">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($t_keg_master_delete->RecKeys as $key) { ?>
...
<?php if ($t_keg_master->tgl->Visible) { // tgl ?>
		<th><span id="elh_t_keg_master_tgl" class="t_keg_master_tgl"><?php echo $t_keg_master->tgl->FldCaption() ?></span></th>
<?php } ?>
<?php if ($t_keg_master->shift->Visible) { // shift ?>
		<th><span id="elh_t_keg_master_shift" class="t_keg_master_shift"><?php echo $t_keg_master->shift->FldCaption() ?></span></th>
<?php } ?>
<?php if ($t_keg_master->hasil->Visible) { // hasil ?>
		<th><span id="elh_t_keg_master_hasil" class="t_keg_master_hasil"><?php echo $t_keg_master->hasil->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$t_keg_master_delete->RecCnt = 0;
$i = 0;
while (!$t_keg_master_delete->Recordset->EOF) {
	$t_keg_master_delete->RecCnt++;
	$t_keg_master_delete->RowCnt++;

	// Set row properties
	$t_keg_master->ResetAttrs();
	$t_keg_master->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$t_keg_master_delete->LoadRowValues($t_keg_master_delete->Recordset);
...
<span id="el<?php echo $t_keg_master_delete->RowCnt ?>_t_keg_master_tgl" class="t_keg_master_tgl">
...
<span id="el<?php echo $t_keg_master_delete->RowCnt ?>_t_keg_master_shift" class="t_keg_master_shift">
...
<span id="el<?php echo $t_keg_master_delete->RowCnt ?>_t_keg_master_hasil" class="t_keg_master_hasil">
...
<?php
	$t_keg_master_delete->Recordset->MoveNext();
}
$t_keg_master_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $t_keg_master_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
ft_keg_masterdelete.Init();
</script>
<?php
$t_keg_master_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$t_keg_master_delete->Page_Terminate();
?>
