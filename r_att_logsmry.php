...
//		$item->Visible = TRUE;
	...
	function Page_Main() {
		...
		$this->scan_date_tgl->SelectionList = "";
		$this->scan_date_tgl->DefaultSelectionList = "";
		$this->scan_date_tgl->ValueList = "";
	...
	function ProcessAjaxPopup() {
		...
		if (@$_GET["popup"] <> "") {
			...
			// Check popup name
			// Build distinct values for scan_date_tgl

			if ($popupname == 'r_att_log_scan_date_tgl') {
				ewr_SetupDistinctValuesFromFilter($this->scan_date_tgl->ValueList, $this->scan_date_tgl->AdvancedFilters); // Set up popup filter
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;

				// Call Page Filtering event
				$this->Page_Filtering($this->scan_date_tgl, $sFilter, "popup");
				$sSql = ewr_BuildReportSql($this->scan_date_tgl->SqlSelect, $this->getSqlWhere(), $this->getSqlGroupBy(), $this->getSqlHaving(), $this->scan_date_tgl->SqlOrderBy, $sFilter, "");
				$rswrk = $conn->Execute($sSql);
				while ($rswrk && !$rswrk->EOF) {
					$this->scan_date_tgl->setDbValue($rswrk->fields[0]);
					$this->scan_date_tgl->ViewValue = @$rswrk->fields[1];
					if (is_null($this->scan_date_tgl->CurrentValue)) {
						$bNullValue = TRUE;
					} elseif ($this->scan_date_tgl->CurrentValue == "") {
						$bEmptyValue = TRUE;
					} else {
						ewr_SetupDistinctValues($this->scan_date_tgl->ValueList, $this->scan_date_tgl->CurrentValue, $this->scan_date_tgl->ViewValue, FALSE, $this->scan_date_tgl->FldDelimiter);
					}
					$rswrk->MoveNext();
				}
				if ($rswrk)
					$rswrk->Close();
				if ($bEmptyValue)
					ewr_SetupDistinctValues($this->scan_date_tgl->ValueList, EWR_EMPTY_VALUE, $ReportLanguage->Phrase("EmptyLabel"), FALSE);
				if ($bNullValue)
					ewr_SetupDistinctValues($this->scan_date_tgl->ValueList, EWR_NULL_VALUE, $ReportLanguage->Phrase("NullLabel"), FALSE);
				$fld = &$this->scan_date_tgl;
			}
	...
	function SetupPopup() {
		...
		// Get 'reset' command
			...
			if (strtolower($sCmd) == "reset") {
				$this->ClearSessionSelection('scan_date_tgl');
				$this->ResetPager();
			}
		}

		// Load selection criteria to array
		// Get scan_date_tgl selected values

		if (is_array(@$_SESSION["sel_r_att_log_scan_date_tgl"])) {
			$this->LoadSelectionFromSession('scan_date_tgl');
		} elseif (@$_SESSION["sel_r_att_log_scan_date_tgl"] == EWR_INIT_VALUE) { // Select all
			$this->scan_date_tgl->SelectionList = "";
		}
	}
	...
	function GetExtendedFilter() {
		...
		if ($bPostBack) {
			...
			// Clear extended filter for field scan_date_tgl
			if ($this->ClearExtFilter == 'r_att_log_scan_date_tgl')
		...
		if ($bSetupFilter) {
			...
			// Field scan_date_tgl
			$sWrk = "";
			$this->BuildExtendedFilter($this->scan_date_tgl, $sWrk);
			ewr_LoadSelectionFromFilter($this->scan_date_tgl, $sWrk, $this->scan_date_tgl->SelectionList);
			$_SESSION['sel_r_att_log_scan_date_tgl'] = ($this->scan_date_tgl->SelectionList == "") ? EWR_INIT_VALUE : $this->scan_date_tgl->SelectionList;
		}
	...
	function LoadDefaultFilters() {
		...
		*/
		...
		$sWrk = "";
		$this->BuildExtendedFilter($this->scan_date_tgl, $sWrk, TRUE);
		ewr_LoadSelectionFromFilter($this->scan_date_tgl, $sWrk, $this->scan_date_tgl->DefaultSelectionList);
		if (!$this->SearchCommand) $this->scan_date_tgl->SelectionList = $this->scan_date_tgl->DefaultSelectionList;
		/**
		* Set up default values for popup filters
		*/

		// Field scan_date_tgl
		// $this->scan_date_tgl->DefaultSelectionList = array("val1", "val2");
	...
	function CheckFilter() {
		...
		// Check scan_date_tgl popup filter
		if (!ewr_MatchedArray($this->scan_date_tgl->DefaultSelectionList, $this->scan_date_tgl->SelectionList))
			return TRUE;
		return FALSE;
	}
	...
	function ShowFilterList($showDate = FALSE) {
		...
		if (is_array($this->scan_date_tgl->SelectionList))
			$sWrk = ewr_JoinArray($this->scan_date_tgl->SelectionList, ", ", EWR_DATATYPE_DATE, 0, $this->DBID);
		$sFilter = "";
		if ($sExtWrk <> "")
			$sFilter .= "<span class=\"ewFilterValue\">$sExtWrk</span>";
		elseif ($sWrk <> "")
	...
	function GetFilterList() {
		...
		if ($sWrk == "") {
			$sWrk = ($this->scan_date_tgl->SelectionList <> EWR_INIT_VALUE) ? $this->scan_date_tgl->SelectionList : "";
			if (is_array($sWrk))
				$sWrk = implode("||", $sWrk);
			if ($sWrk <> "")
				$sWrk = "\"sel_scan_date_tgl\":\"" . ewr_JsEncode2($sWrk) . "\"";
		}
		if ($sWrk <> "") {
			if ($sFilterList <> "") $sFilterList .= ",";
			$sFilterList .= $sWrk;
	...
	function SetupFilterList($filter) {
		...
		if (array_key_exists("sel_scan_date_tgl", $filter)) {
			$sWrk = $filter["sel_scan_date_tgl"];
			$sWrk = explode("||", $sWrk);
			$this->scan_date_tgl->SelectionList = $sWrk;
			$_SESSION["sel_r_att_log_scan_date_tgl"] = $sWrk;
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "scan_date_tgl"); // Clear extended filter
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "scan_date_tgl");
			$this->scan_date_tgl->SelectionList = "";
			$_SESSION["sel_r_att_log_scan_date_tgl"] = "";
		}
		return TRUE;
	}
	...
	function GetPopupFilter() {
		...
		if ($this->DrillDown)
			return "";
		if (!$this->ExtendedFilterExist($this->scan_date_tgl)) {
			if (is_array($this->scan_date_tgl->SelectionList)) {
				$sFilter = ewr_FilterSQL($this->scan_date_tgl, "`scan_date_tgl`", EWR_DATATYPE_DATE, $this->DBID);

				// Call Page Filtering event
				$this->Page_Filtering($this->scan_date_tgl, $sFilter, "popup");
				$this->scan_date_tgl->CurrentFilter = $sFilter;
